 ...
 * 設定漏れが「動かない」として現れる方が無警告の公開より安全
 */
export type AuthMiddleware = MiddlewareHandler;

/** 長さも含めた定数時間比較, タイミング差からの漏洩を防止 */
...
export type BearerOptions = {
	 ...
	 * HTMLは未認証でも返り表示自体には不要, ブラウザからAPIへAuthorizationを付けられない場合に必要
	 * cookieで受ける以上CSRFの対象になり、発行側でSameSite=Strictを付けること
	 */
	cookie?: string;
};
...
export function readCookie(header: string | undefined, name: string): string | undefined {
	...
	for (const part of header.split(';')) {
		...
		// 書き出し側のencodeURIComponentを復号, 未復号ではbase64の`+ / =`が不一致
		try {
			return decodeURIComponent(value);
		} catch {
			// 不正な%列は素の値で比較, 例外では401ではなく500
			return value;
		}
	}
	return undefined;
...
/**
 * 認証を行わずREST APIとダッシュボードを開放(ADR-0044)
 *
 * Workerへ到達できる全員が、ジョブの内容の閲覧と投入と取り消しを実行可能
 * 手前で認証している構成と`wrangler dev`での確認に限って使用
 * 未設定の場合のfail-closedは変わらない, 開放するには明示的にこれを渡す必要がある
 */
export function unsafeNoAuth(): AuthMiddleware {
	let warned = false;
	return async (_c, next) => {
		// isolateごとに1回だけ, 毎リクエストのログを回避
		if (!warned) {
			warned = true;
...
/** `env`からトークンを取得する関数, secretはモジュール初期化時に読めない */
export type TokenResolver = (env: any) => string | undefined;

/**
 * シークレット1つで始められる最短の経路
 *
 * 関数を渡すとリクエストごとに`env`から取得
 * Cloudflareのsecretは`env`経由でしか読めず、直接記述の回避にはこの形式が必要
 * 解決できなければ拒否, 設定漏れでの無認証通過を防止(ADR-0013)
 */
