 ...
 * - `anyOf: [{type}, {type: 'null'}]`は3.1のtype配列へ畳む
 * - `unevaluatedProperties`は`additionalProperties`へ(Recordの表現, 対応する生成器が広い)
 * - 既定値と同じ`explode: false` / `required: false`と空のparametersは落とす
 */
...
const normalize = (value) => {
	...
	for (const [key, entry] of Object.entries(value)) {
		if (key === 'explode' && entry === false) continue;
		...
		if (key === 'anyOf' && isNullUnion(value)) {
			out.type = [entry.find((option) => option.type !== 'null').type, 'null'];
			continue;
		}
