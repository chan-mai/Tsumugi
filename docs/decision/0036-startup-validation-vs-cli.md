...
その起動時検証の範囲を定めないままのため、CLIとの分担が決まらず双方とも着手できていない
...
マイグレーションの適用そのものは`wrangler d1 migrations apply`が担い、検証は未適用の検知に留める
...
Workersに起動フックが無いため「初回の呼び出しを起動とみなす」形になり、検証は最初のリクエストまで実行されない
検証できるのは`env`に見える範囲に限られ、wrangler設定そのものとの突き合わせはできない

CLIが生成する断片と起動時検証が返す断片は同じ実装から作る, 2箇所で書くと片方だけが古くなる
