...
OpenAPI文書の原本をTypeSpec(`packages/spec`)へ移す

- `.tsp`がモデルと経路を持ち、`tsp compile`(`@typespec/openapi3`)がOpenAPI 3.1のJSONを生成する
- 生成物は`@tsumugi/dashboard`と同じ方式で配る。`emit-module.mjs`が`dist/index.js`の定数へ埋め込み、`prepare`がビルドし、tsdownの`noExternal`でバンドルへ統合する
- 生成器の表現差はemit時に従来の形へ正規化する。nullableの`anyOf`はtype配列へ,`unevaluatedProperties`は`additionalProperties`へ,スカラーのクエリパラメータで意味を持たない`explode: false`と既定値と同じ`required: false`と空の`parameters`は除去する。意味が同じでも表現が変わると、利用者のクライアント生成物が移行だけで変わるため
...
- 経路の網羅, 503の記載, `$ref`の整合, 並べ替え列の一致を見る既存の単体テストは、生成した文書に対してそのまま機能する
- `api/types.ts`と文書の統合は範囲外。TypeSpecからの型生成へ寄せる余地は残る
- 定義の変更にはビルド(`pnpm --filter @tsumugi/spec build`)が必要。`prepare`が自動で行うため,通常の開発手順は変わらない
