...
- performerの依存(重いSDK等)をジョブ管理Worker本体から分離する
...
consumerは印で判別してコンストラクタ呼び出しとRPC呼び出しを切り替える
