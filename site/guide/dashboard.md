...
詳細画面に`ctx.log()`で保存したメッセージが受信順で表示されます。
各記録から試行回数、記録時刻、本文を確認できます。実行中のログも対象ですが、反映には遅延があります。

ログはリトライを含む1ジョブ全体で最新の20件まで保存され、本文はJavaScriptの文字列長で2,000文字までです。
保存間隔の下限は1,000msです。
保持期間はジョブと同じ設定に従います。

投入時に`traceparent`を指定したジョブでは、その値を詳細画面で確認可能です。
performerでの使用方法は[ログ](/guide/performer#ログ)と[トレース情報](/guide/performer#トレース情報)を参照してください。
