...
全ての時刻にIANAタイムゾーンと、その時点のGMTオフセットが含まれます。DST終了時に同じローカル時刻が2回存在してもオフセットで区別できます。

作成日時の範囲入力はブラウザのローカル日付です。選択した日の0時から23時59分59.999秒までを同じブラウザタイムゾーンで検索します。
...
実行可否を事前に表示するための近似値であり、実際の可否は要求の時点で判定されます。
