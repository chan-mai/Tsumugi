...
`sort`と`order`に不正な値が来ても400にはせず、既定値を採用します。ダッシュボードの表示が止まらないようにするためです。
...
一覧への反映は数秒遅れるため、この値は見積りです。`ids`で指定した場合は常に0です。

対象は一覧の条件で選びますが、状態は要求の時点で改めて判定します。
一覧に表示されていても、その時点で条件に合わないジョブは`failed`に含まれます。
...
`oldestScheduledMs`は最も古い`SCHEDULED`のジョブが投入から待機している時間です。対象が無い場合はnullです。
...
一部のshardにのみ適用された場合は、変更と同じく500と`failed`が返ります。
...
| `last_job_id`     | 直近の発火で投入したジョブ。`kind`が`flow`の場合は`null` |
| `last_run_id`     | 直近の発火で開始したRun。`kind`が`job`の場合は`null`     |
| `last_skipped_at` | 直近で発火しなかった時刻。一度も該当がない場合は`null`   |
...
```json
{ "id": "REPORT:nightly-1767650000000-manual", "kind": "flow" }
```

`id`は投入したジョブのIDまたは開始したRunのIDで、`kind`はその区別です。

一時停止中でも可能で、`overlap`の判定は行いません。次回の定期の発火時刻への影響はありません。
発火に失敗した場合は500とエラー詳細がthrowされ、一覧の`last_error`にも同様の内容が記録されます。
...
| `deadlineMs` | 任意 | Run全体の期限(ミリ秒)。正の整数。超過したRunは中断され`FAILED`になります。Flow定義の期限より優先されます |
